...
namespace retro {
    ...
    namespace ranges {
        ...
        struct ElementsOf {
            ...
            constexpr ElementsOf(R&& range, A&& alloc) noexcept : range(static_cast<R &&>(range)), alloc(static_cast<A &&>(alloc)) {}
    ...
    struct GeneratorPromiseBase
    {
        ...
        struct FinalAwaiter {
            ...
            await_suspend(std::coroutine_handle<P> handle) noexcept {
        ...
        YieldSequenceAwaiter<Generator<T, std::remove_cvref_t<T>, A>>
        yield_value(ranges::ElementsOf<R, A> && x) {
    ...
    struct GeneratorPromise<Generator<T, V, A>, B, ExplicitAllocator> final
        : public GeneratorPromiseBase<T>
...
namespace std {
    ...
    public:
        using promise_type = retro::GeneratorPromise<retro::Generator<R, V>, ByteAllocator, true /*explicit Allocator*/>;
    };
...
namespace retro {
    ...
    public:
        ...
        private:
            ...
            explicit Iterator(CoroutineHandle coroutine) noexcept
            : coroutine(coroutine) {}
    ...
    private:
        explicit Generator(CoroutineHandle coroutine) noexcept
            : coroutine(coroutine) {
        }

    public: // to get around access restrictions for __yield_sequence_awaitable
        std::coroutine_handle<> get_coro() noexcept { return coroutine; }
        promise_type* get_promise() noexcept { return std::addressof(coroutine.promise()); }
    ...
    private:
        ...
        explicit Generator(std::coroutine_handle<P> coroutine) noexcept
            : promise(std::addressof(coroutine.promise()))
            , coroutine(coroutine)
        {}

    public: // to get around access restrictions for __yield_sequence_awaitable
        std::coroutine_handle<> get_coro() noexcept { return coroutine; }
        PromiseBase* get_promise() noexcept { return promise; }
