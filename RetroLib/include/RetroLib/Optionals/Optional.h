...
namespace retro {
    ...
    struct OptionalStorage {
        ...
        constexpr explicit OptionalStorage(std::in_place_type_t<T>, A&&... args) noexcept(std::is_nothrow_constructible_v<T, A...>) : data(std::forward<A>(args)...), is_set(true) {}
    ...
    struct OptionalBase : private OptionalStorage<T> {
        ...
        constexpr void swap(OptionalBase& other) noexcept(std::is_nothrow_move_constructible_v<T> && std::is_nothrow_swappable_v<T>) requires std::swappable<T> {
            constexpr bool can_swap_trivially = std::is_trivially_move_constructible_v<T> && std::is_trivially_move_assignable_v<T>;
    ...
    protected:
        ...
        template <typename U>
        constexpr void assign_from(U&& other) noexcept(std::is_nothrow_constructible_v<T, decltype(*std::forward<U>(other))> && std::is_nothrow_assignable_v<T&, decltype(*std::forward<U>(other))>) {
            if (!other.has_value()) {
                reset();
            } else if (is_set) {
    ...
    template <ValidOptionalType T>
    using CopyConstructLayer = std::conditional_t<std::is_copy_constructible_v<T> && !std::is_trivially_copy_constructible_v<T>,
    ...
    struct OptionalCopyAssign : MoveConstructLayer<T> {
        ...
        OptionalCopyAssign& operator=(const OptionalCopyAssign& other) noexcept(std::is_nothrow_copy_assignable_v<T> && std::is_nothrow_constructible_v<T, decltype(*other)>) {
    ...
    struct OptionalMoveAssign : CopyAssignLayer<T> {
        ...
        OptionalMoveAssign& operator=(OptionalMoveAssign&& other) noexcept(std::is_nothrow_move_constructible_v<T> && std::is_nothrow_move_assignable_v<T>) {
    ...
    public:
        ...
        constexpr explicit(!std::convertible_to<U, T>) Optional(U&& value) : Base(std::in_place_type<T>, std::forward<U>(value)) {}
        ...
        template <ValidOptionalType U = T>
            requires (!std::same_as<std::decay_t<U>, Optional> && std::constructible_from<T, U> && std::assignable_from<T&, U>)
        constexpr Optional& operator=(U&& other) noexcept(std::is_nothrow_constructible_v<T, U> && std::is_nothrow_assignable_v<T&, U>) {
            if (has_value()) {
        ...
        T& emplace(std::initializer_list<U> initializer_list, A&&... args) noexcept(std::is_nothrow_constructible_v<T, std::initializer_list<U>, A...>) {
            reset();
        ...
        template <ValidOptionalType U>
        constexpr bool operator==(const Optional<U>& other) const noexcept(noexcept(**this == *other)) {
            return has_value() == other.has_value() && (!has_value() || **this == *other);
        }

        template <ValidOptionalType U>
        constexpr auto operator<=>(const Optional<U>& other) const noexcept(noexcept(**this <=> *other)) {
        ...
        friend constexpr bool operator==(std::nullopt_t, const Optional& other) noexcept {
            return !other.has_value();
        }

        friend constexpr std::strong_ordering operator<=>(std::nullopt_t, const Optional& other) noexcept {
